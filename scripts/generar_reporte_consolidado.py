...
BD_FORMACION = os.environ.get(
    "BD_FORMACION", r"C:\ws\sena\data\PE-04\sena_formacion_septiembre.db"
)
CUPOS_DISPONIBLES = os.environ.get(
    "CUPOS_DISPONIBLES",
    DIR_BASE / "metas" / "cupos_disponibles_por_regional_2025.xlsx",
)
REPORTE_APRENDICES = os.environ.get(
    "REPORTE_APRENDICES",
    rf"C:\ws\sena\data\aprendices\SENA Mensual Nacional {MES_CORTO} {ANIO}.xlsx",
)
ARCHIVO_SALIDA = os.environ.get(
    "ARCHIVO_SALIDA",
    rf"C:\ws\sena\data\REPORTE_ECONOMIA_NARANJA\Reporte Consolidado Economía Naranja {MES_CORTO} {ANIO}.xlsx",
)
    ...
    if "Economía Naranja" in nombre:
        ...
        for long_mes in range(len(MES_TRABAJO), 2, -1):
            ...
            if len(nombre_nuevo) <= max_length:
                print(
                    f"   [INFO] Nombre de hoja recortado: '{nombre}' -> '{nombre_nuevo}'"
                )
                return nombre_nuevo
...
if os.path.exists(db_economia_naranja):
    ...
    try:
        ...
        if tabla_existe:
            ...
            print(
                "   Por favor ejecute primero el paso 4 para crear la tabla precalculada"
            )
            conn_eco.close()
            sys.exit(1)
        ...
        df_economia_naranja = pd.DataFrame(
            columns=[
...
if not os.path.exists(archivo_cupos_disponibles):
    ...
    print("   [INFO] Ejecute primero: python metas/cruce_metas_avance_final.py")
    sys.exit(1)
...
print("   [OK] Hoja copiada con formato preservado")
...
print("   [OK] Todos los formatos aplicados correctamente")
...
print(
    f"   Hoja 3 '{nombre_hoja3}': Copiada desde {os.path.basename(archivo_aprendices)} (con formato)"
)
